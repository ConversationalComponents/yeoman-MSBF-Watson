...
module.exports = class extends Generator {
  ...
  initializing() {
    ...
    this.log(
      "\nDetailed documentation can be found at " +
        chalk.underline("https://aka.ms/botbuilder-generator\n")
    );
  }
  ...
  install() {
    ...
    if (this.templateConfig.finalConfirmation === true) {
      ...
      this.log(
        "Thank you for using the Microsoft Bot Framework with Conversational Components. "
      );
  ...
  _verifyNoPromptOptions() {
    this.templateConfig = _.pick(this.options, [
      "botname",
      "description",
      "language",
      "template",
    ...
    if (!this.templateConfig.botname) {
      throw new Error(
        "Must specify a name for your bot when using --noprompt argument.  Use --botname or -N"
      );
    }
    if (!this.templateConfig.description) {
      throw new Error(
        "Must specify a description for your bot when using --noprompt argument.  Use --description or -D"
      );
    }

    // make sure we have a supported language
    const language = this.templateConfig.language
      ? _.toLower(this.templateConfig.language)
      : undefined;
    ...
    if (!language || (language !== langJS && language !== langTS)) {
      throw new Error(
        "Must specify a programming language when using --noprompt argument.  Use --language or -L"
      );
    }
